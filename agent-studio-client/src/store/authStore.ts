  ...
  persist(
    (set, get) => ({
      ...
      logout: () => {
        ...
        // Also clear any agentstudio:* keys in localStorage
        try {
          const keysToRemove: string[] = [];
          for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key && (key.startsWith('agentstudio:') || key.startsWith('agentstudio-chats-') || key.startsWith('agentstudio-pref-'))) {
