...
frontend (`frontend/agentstudio-frontend/`) for **AgentStudio**, a RAG document
...
(e.g. `agentstudio:{userId}:conversations`, `agentstudio:{userId}:messages:{conversationId}`),
...
1. **`store/authStore.ts`** (Zustand, persisted via the `persist` middleware
   into `localStorage` under one key, e.g. `agentstudio-auth`): state
...
`frontend/agentstudio-frontend/`. Keep the existing dark slate/indigo visual
...
9. **Branding polish**: replace the default Vite favicon/title
   (`index.html`) with AgentStudio branding consistent with the in-app logo
